...
const FurnitureListWithPaginationContainer = createPaginationContainer(
  FurnitureList,
  {
    viewer: graphql`
      fragment furnitureList_viewer on Viewer @argumentDefinitions(
        first: { type: "Int", defaultValue: 10 }
        after: { type: "String" }
        category: { type: "String", defaultValue: "furniture" }
      ) {
        commodities(first: $first, after: $after, category: $category)
          @connection(key: "furnitureList_commodities", filters: []) {
          pageInfo {
            startCursor
            endCursor
            hasNextPage
  ...
  },
  {
    ...
    query: graphql`
      query furnitureListPaginationQuery($category: String, $first: Int, $after: String) {
        viewer {
          ...furnitureList_viewer @arguments(category: $category, first: $first, after: $after)
        }
      }
    `,
  },
...
export default FurnitureListWithPaginationContainer;
